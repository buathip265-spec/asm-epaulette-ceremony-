...
import { 
  ...
  DownloadCloud, Share2, Layers, SmartphoneCharging
} from 'lucide-react';
...
// ============================================================================
// ⚙️ ส่วนตั้งค่า FIREBASE (เชื่อมต่อกับโปรเจกต์ ASM Epaulette Ceremony เรียบร้อยแล้ว)
// ============================================================================
...
const activeFirebaseConfig = (() => {
  ...
  if (CUSTOM_FIREBASE_CONFIG.projectId) {
    return CUSTOM_FIREBASE_CONFIG;
  }
  return {
    apiKey: "demo-key",
...
const FIXED_APP_ID = 'ceremony-honors-2026';
const appId = typeof __app_id !== 'undefined' ? __app_id : FIXED_APP_ID;
...
export default function App() {
  ...
  // ระบบสั่นมือถือ (Haptic Feedback)
  ...
  useEffect(() => {
    ...
    const guestsColRef = collection(db, 'artifacts', appId, 'public', 'data', 'guests');
  ...
  const getGuestDocRef = (guestId) => {
    return doc(db, 'artifacts', appId, 'public', 'data', 'guests', guestId);
  };
  ...
  const handleConfirmImportExcel = async () => {
    ...
    try {
      ...
      if (user && syncStatus === 'connected') {
        ...
        if (importMode === 'replace') {
          setGuests(sortAndAssignBadges(excelPreviewData));
        } else {
          setGuests((prev) => sortAndAssignBadges([...prev, ...excelPreviewData]));
        }
      }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-100 text-slate-800 font-sans select-none pb-20 md:pb-0">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-3 sm:p-5">
        ...
        {/* Modal แนะนำวิธีติดตั้งเป็นแอป (PWA / APK) */}
        {isAppModalOpen && (
